...
def next_topk_coord(
    array: torch.Tensor, used_inds: List, available_batches: List, k: int = 1
    ...
    new_inds = [
        np.unravel_index(i, shape=(array.size()[2], array.size()[3]))
        for i in inds.cpu().numpy()
    ]
    chosen_inds = []
    for k_ind in range(k):
        chosen_inds = chosen_inds + [
            [i, j[0][k_ind], j[1][k_ind]] for i, j in enumerate(new_inds)
        ]
...
def project_L0_box_torch(x_input: torch.Tensor, k: int, lb: float, ub: float) -> torch.Tensor:
    ...
    x_output = (
        x_output * (torch.logical_and(lb <= x_output, x_output <= ub))
        + lb * (lb > x_output)
        + ub * (x_output > ub)
    )
...
def get_expl(
    ...
    assert (
        expl_method in expl_methods.keys()
    ), f"explanation method {expl_method} is not supported."
    ...
    if expl_method == "smooth_grad":
        ...
        heatmap = smooth_explanation_method.attribute(
            x,
            nt_samples=10,
            stdevs=sigma,
            nt_type="smoothgrad",
...
def topk_intersect(x: torch.Tensor, y: torch.Tensor, k: int) -> float:
    ...
    intersect = (
        float(
            len(
                np.intersect1d(
                    topk_x_ind.cpu().detach().numpy(), topk_y_ind.cpu().detach().numpy()
                )
            )
        )
        / k
