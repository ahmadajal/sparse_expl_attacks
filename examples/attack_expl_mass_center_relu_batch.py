...
argparser.add_argument('--output_dir', type=str, default='output_expl_mass_center_relu/', help='directory to save results to')
...
print("sigma: ", sigma)
## expl loss
class EXPL_Loss_mass_center(lf.PartialLoss):
    def __init__(self, classifier, org_expl_mass_center, x_mesh, y_mesh, method, normalizer=None, smooth=False, multiply_by_input=False):
        super(EXPL_Loss_mass_center, self).__init__()
        self.classifier = classifier
        self.org_expl_mass_center = org_expl_mass_center
        self.x_mesh = x_mesh
        self.y_mesh = y_mesh
        self.method = method
    ...
    def forward(self, examples, labels, *args, **kwargs):
        ...
        #############
        if BATCH_SIZE==1:
            adv_expl_mass_center = [torch.sum(adv_expl*self.x_mesh), torch.sum(adv_expl*self.y_mesh)]
            loss_expl = ((self.org_expl_mass_center[0]-adv_expl_mass_center[0])**2 + (self.org_expl_mass_center[1]-adv_expl_mass_center[1])**2)
            print("expl loss:", loss_expl)
        else:
            adv_expl_mass_center = [torch.sum(adv_expl*self.x_mesh, dim=(1,2,3)),
            torch.sum(adv_expl*self.y_mesh, dim=(1,2,3))]
            loss_expl = ((self.org_expl_mass_center[0]-adv_expl_mass_center[0])**2 + (self.org_expl_mass_center[1]-adv_expl_mass_center[1])**2)
            loss_expl = torch.mean(loss_expl)
            print("expl loss:", loss_expl)
        return -1 * loss_expl # we want this loss to be big!
...
# finding the mass center of the original explanation
...
if BATCH_SIZE == 1:
    ...
    org_expl_mass_center = [int(torch.sum(org_expl*x_mesh)), int(torch.sum(org_expl*y_mesh))]
else:
    y_mesh, x_mesh = np.meshgrid(np.arange(224),np.arange(224))
    x_mesh = torch.Tensor(x_mesh).unsqueeze(0).repeat(BATCH_SIZE,1,1,1).to(device)
    y_mesh = torch.Tensor(y_mesh).unsqueeze(0).repeat(BATCH_SIZE,1,1,1).to(device)
    org_expl_mass_center = [torch.sum(org_expl*x_mesh, dim=(1,2,3), dtype=torch.int),
    torch.sum(org_expl*y_mesh, dim=(1,2,3), dtype=torch.int)]
print(org_expl_mass_center)
...
# explanation loss
expl_loss = EXPL_Loss_mass_center(model, org_expl_mass_center, x_mesh, y_mesh, args.method, normalizer, smooth=args.smooth, multiply_by_input=args.multiply_by_input)
...
attack_loss = lf.RegularizedLoss({'expl': expl_loss, 'smooth': smooth_loss},
                                 {'expl': 1.0 , 'smooth': 0.05},  # lambda = 0.05
